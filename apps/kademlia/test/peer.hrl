...
should_store_data({{PeerPid, _Id} = Peer, KbucketPid}) ->
    FakePeer = ?FAKE_PEER,
...
should_overwrite_data_with_same_key({{PeerPid, _Id} = Peer, KbucketPid}) ->
    FakePeer = ?FAKE_PEER,
...
should_contact_the_kbucket_for_its_closest_peer_to_a_key({{PeerPid, _Id} = Peer, KbucketPid}) ->
    FakePeer = ?FAKE_PEER,
    KeyToSearch = 2,
